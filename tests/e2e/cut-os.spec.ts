...
test('Cut OS command is visible in the first dashboard viewport', async ({ page }) => {
  ...
  await expect(commandCard.getByTestId('cut-os-proof-strip')).toBeVisible()
  await expect(commandCard.getByTestId('proof-token-scale')).toBeVisible()
  await expect(commandCard.getByTestId('proof-token-training')).toBeVisible()
  await expect(commandCard.getByTestId('proof-token-food')).toBeVisible()
  await expect(commandCard.getByTestId('proof-token-phase')).toBeVisible()
